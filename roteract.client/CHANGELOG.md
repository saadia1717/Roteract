...
- Create Angular project with ng: `ng new roteract.client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`roteract.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `roteract.client.esproj`).
